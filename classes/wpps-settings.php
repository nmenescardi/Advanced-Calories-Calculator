...
if ( ! class_exists( 'WPPS_Settings' ) ) {
	...
	class WPPS_Settings extends WPPS_Module {
		 ...
		 * @param array  $value This will be merged with WPPS_Settings->settings, so it should mimic the structure of the WPPS_Settings::$default_settings. It only needs the contain the values that will change, though. See Advanced_Calories_Calculator->upgrade() for an example.
		 */
		...
		public function register_hook_callbacks() {
			...
			add_action( 'wp',               __CLASS__ . '::register_front_end_form' );
		...
		/**
		 * Adds a Front-End page with the Calculator Form
		 *
		 * @mvc Controller
		 */
		public static function register_front_end_form() {
				
				if ( is_page( 'advanced-calories-calculator' ) ) {	
				
				
					echo self::render_template( 'calculator-form.php' );
		...
		public function validate_settings( $new_settings ) {
			...
			if ( ! is_string( $new_settings['db-version'] ) ) {
				$new_settings['db-version'] = Advanced_Calories_Calculator::VERSION;
			}
